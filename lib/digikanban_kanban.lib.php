...
function kanban_prepare_head(Kanban $object): array
{
	...
	return saturne_object_prepare_head($object, $head, $moreparam, false);
}

function get_kanban_linkable_objects(): array
{
...
//	}
	...
	if (isModEnabled('digiquali')) {
		$linkableObjectTypes['control'] = [
			'langs'          => 'Control',
			'langfile'       => 'object@digiquali',
			'picto'          => 'control',
			'className'      => 'Control',
			...
			'name_field'     => 'ref',
			'create_url'     => 'custom/digiquali/control/card.php?',
			'class_path'     => 'custom/digiquali/class/control.class.php',
		];
	}
...
//    }
	...
	$hookmanager->initHooks(['get_kanban_linkable_objects']);

	$resHook = $hookmanager->executeHooks('extendKanbanLinkableObjectsList', $linkableObjectTypes);
	...
	if (is_array($linkableObjectTypes) && !empty($linkableObjectTypes)) {
		foreach($linkableObjectTypes as $linkableObjectType => $linkableObjectInformations) {
			if ($linkableObjectType != 'context' && $linkableObjectType != 'currentcontext') {
				...
				$confCode = 'DIGIKANBAN_KANBAN_LINK_' . strtoupper($linkableObjectType);
