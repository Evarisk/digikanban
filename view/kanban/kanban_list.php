...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'kanbanlist'; // To manage different context of search
...
$hookmanager->initHooks(array('kanbanlist')); // Note that conf->hooks_modules contains array
...
if (!empty($conf->categorie->enabled)) {
	$search_category_array = GETPOST("search_category_kanban_list", "array");
}
...
$linkableElements = get_kanban_linkable_objects();
...
$permissiontoadd    = $user->rights->digikanban->kanban->write;
$permissiontodelete = $user->rights->digikanban->kanban->delete;
...
if (empty($reshook)) {
		...
		if (!empty($toselect)) {
			foreach ($toselect as $toselectedid) {
				...
				if ($object->type == 'UniqueChoice' || $object->type == 'MultipleChoices') {
					$answerList = $answer->fetchAll('ASC', 'position', 0, 0, ['fk_kanban' => $object->id]);
				}
				if (($object->type != 'UniqueChoice' && $object->type != 'MultipleChoices') || !empty($answerList)) {
					...
					$search_array_options = [];
					$action               = 'list';
					setEventMessages($langs->trans("LockKanban", $object->ref), []);
				} else {
					// Set locked KO
		...
		foreach ($toselect as $toselectid) {
			...
			if ($result > 0) {
				if (method_exists($objecttmp, 'isErasable') && $objecttmp->isErasable() <= 0) {
					$langs->load("errors");
					$nbignored++;
					$TMsg[] = '<div class="error">'.$langs->trans('ErrorKanbanUsedInSheet',$objecttmp->ref).'</div><br>';
					continue;
				}
	...
	if (!$error && ($massaction == 'add_kanbans' || ($action == 'add_kanbans' && $confirm == 'yes')) && $permissiontoadd) {
		if (is_array($toselect) && !empty($toselect)) {
			$totalKanbans  = 0;
			$kanbanInArray = [];
			if (GETPOSTISSET('sheet') && GETPOST('sheet') > 0) {
				$sheet->fetch(GETPOST('sheet'));
				$sheet->fetchObjectLinked($sheet->id, 'digikanban_' . $sheet->element, null, 'digikanban_' . $object->element, 'OR', 1, 'position');
				foreach ($toselect as $selected) {
					$object->fetch($selected);
					if (is_array($sheet->linkedObjectsIds['digikanban_' . $object->element]) && !empty($sheet->linkedObjectsIds['digikanban_' . $object->element]) && in_array($object->id, $sheet->linkedObjectsIds['digikanban_' . $object->element])) {
						$kanbanInArray[] = $object->getNomUrl(1, 'nolink', 1);
					} else {
						$totalKanbans++;
						$object->add_object_linked('digikanban_' . $sheet->element, GETPOST('sheet'));
						$kanbanIds   = $sheet->linkedObjectsIds['digikanban_' . $object->element];
						$kanbanIds[] = $object->id;
						$sheet->updateKanbansPosition($kanbanIds);
					}
				}
...
$now      = dol_now();
$help_url = '';
$title    = $langs->trans('ListOf', $langs->transnoentitiesnoconv("Kanbans"));
...
if (!empty($conf->categorie->enabled)) {
//	$sql .= Categorie::getFilterJoinQuery('kanban', "t.rowid");
}
...
foreach ($search as $key => $val) {
	if ($key == 'status' && $val == 'specialCase') {
		$newStatus = [Kanban::STATUS_VALIDATED, Kanban::STATUS_LOCKED];
		if (!empty($newStatus)) {
...
if (!empty($conf->categorie->enabled)) {
//	$sql .= Categorie::getFilterSelectQuery('kanban', "t.rowid", $search_category_array);
}
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $searchAll && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/digikanban/view/kanban/kanban_card.php', 1).'?id='.$id);
	exit;
}
...
$arrayofmassactions['pre_add_kanbans'] = '<span class="fas fa-plus-circle paddingrightonly"></span>' . $langs->transnoentities('AddToSheet');
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/digikanban/view/kanban/kanban_card.php?fromtype=' . $fromtype . '&fromid=' . $fromid, 1).'&action=create', '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

if ($massaction == 'prelock') {
	print $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('ConfirmMassLock'), $langs->trans('ConfirmMassLockingKanban', count($toselect)), 'lock', null, '', 0, 200, 500, 1);
}

if ($massaction == 'prearchive') {
	print $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('ConfirmMassArchive'), $langs->trans('ConfirmMassArchivingKanban', count($toselect)), 'archive', null, '', 0, 200, 500, 1);
}

if ($massaction == 'pre_add_kanbans') {
...
//	$moreforfilter .= $formcategory->getFilterBox('kanban', $search_category_array);
}
